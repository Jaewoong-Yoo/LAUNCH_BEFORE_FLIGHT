...
class Detection:
    def __init__(self):
        ...
        # self.cam0_addr = "/dev/v4l/by-id/usb-Azurewave_USB2.0_HD_UVC_WebCam_0x0001-video-index0"    # cam in laptops
        # Left cam
        self.cam0_addr = "/dev/v4l/by-id/usb-e-con_systems_NileCAM30_USB_14480200-video-index0"  #left 90
        self.cam1_addr = "/dev/v4l/by-id/usb-e-con_systems_NileCAM30_USB_2B460000-video-index0"  #left 45
        self.cam2_addr = "/dev/v4l/by-id/usb-e-con_systems_NileCAM30_USB_114F0B00-video-index0"  #left 0


        # Right cam        
        # self.cam3_addr = "/dev/v4l/by-id/usb-e-con_systems_NileCAM30_USB_19070800-video-index0"	 #right 90
        # self.cam4_addr = "/dev/v4l/by-id/usb-e-con_systems_NileCAM30_USB_0F080800-video-index0"  #right 45
        # self.cam5_addr = "/dev/v4l/by-id/usb-e-con_systems_NileCAM30_USB_1A080800-video-index0"  #right 0
        ###################################################################################################
        ...
        self.OF_CAM3_count   = 0
        self.OF_CAM4_count   = 0
        self.OF_CAM5_count   = 0
    ...
    def img_processing(self, img, CAM_num):
        ...
        if CAM_num==0:
            ...
            if  self.OF_CAM3_count==0:
                self.OF_CAM3_count = 1
                Prev_CMO_img = img.copy()
            else :
                Prev_CMO_img = self.Prev_CMO_img3.copy()
            TRACKS          = self.tracks3
            FRAME_IDX       = self.frame_idx3

        elif CAM_num==4:
            if  self.OF_CAM4_count==0:
                self.OF_CAM4_count = 1
                Prev_CMO_img = img.copy()
            else :
                Prev_CMO_img = self.Prev_CMO_img4.copy()
            TRACKS          = self.tracks4
            FRAME_IDX       = self.frame_idx4

        elif CAM_num==5:
            if  self.OF_CAM5_count==0:
                self.OF_CAM5_count = 1
                Prev_CMO_img = img.copy()
            else :
                Prev_CMO_img = self.Prev_CMO_img5.copy()
            TRACKS          = self.tracks5
            FRAME_IDX       = self.frame_idx5
        ...
        if CAM_num==0:
            ...
            self.Prev_CMO_img3  = CMO_img
            self.tracks3        = TRACKS
            self.frame_idx3     = FRAME_IDX 
        elif CAM_num==4:
            self.Prev_CMO_img4  = CMO_img
            self.tracks4        = TRACKS
            self.frame_idx4     = FRAME_IDX 
        elif CAM_num==5:
            self.Prev_CMO_img5  = CMO_img
            self.tracks5        = TRACKS
            self.frame_idx5     = FRAME_IDX
        img = vis
        return img
    ...
    def Camera_init(self, camnum):  # camera init
        ...
        elif camnum==3:         ## Camera3
            self.cap3 = cv2.VideoCapture(self.cam3_addr)
            self.cap3.set(3,self.ImgWidth)
            self.cap3.set(4,self.ImgHeight)
            self.cap3.set(5,self.ImgFps)
        elif camnum==4:         ## Camera4
            self.cap4 = cv2.VideoCapture(self.cam4_addr)
            self.cap4.set(3,self.ImgWidth)
            self.cap4.set(4,self.ImgHeight)
            self.cap4.set(5,self.ImgFps)
        elif camnum==5:         ## Camera5
            self.cap5 = cv2.VideoCapture(self.cam5_addr)
            self.cap5.set(3,self.ImgWidth)
            self.cap5.set(4,self.ImgHeight)
            self.cap5.set(5,self.ImgFps)
...
#                  zoom_absolute 0x009a090d (int)    : min=100 max=800 step=1 default=102 value=102
    ...
    def run(self):
        ...
        if self.save_img == 1:
            if ASUS:
                saving_folder   = "/home/usrg/Civil/img_test/"
            if STICK:
            ...
            path_image3     = saving_folder+local_folder+'/frame3/'
            path_image4     = saving_folder+local_folder+'/frame4/'
            path_image5     = saving_folder+local_folder+'/frame5/'
            ...
            if not(os.path.isdir('./'+local_folder)):
                ...
                os.makedirs(os.path.join(path_image3))
                os.makedirs(os.path.join(path_image4))
                os.makedirs(os.path.join(path_image5))
        ...
        image_pub3 = rospy.Publisher("image_frame3",        Image,  queue_size=1)
        image_pub4 = rospy.Publisher("image_frame4",        Image,  queue_size=1)
        image_pub5 = rospy.Publisher("image_frame5",        Image,  queue_size=1)
        image_pub6 = rospy.Publisher("image_frame6",        Image,  queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if ret0 is True or ret1 is True or ret2 is True or ret3 is True or ret4 is True or ret5 is True:
                # now = time.time()
                ...
                try:
                    ...
                    if ret3 is True: 
                        image_pub4.publish(bridge.cv2_to_imgmsg(frame3, "bgr8"))  # "bgr8"  "mono8"
                    if ret4 is True: 
                        image_pub5.publish(bridge.cv2_to_imgmsg(frame4, "bgr8"))  # "bgr8"  "mono8"
                    if ret5 is True: 
                        image_pub6.publish(bridge.cv2_to_imgmsg(frame5, "bgr8"))  # "bgr8"  "mono8"
                ...
                if self.save_img == 1:
                    ...
                    if ret2 is True: 
                        ...
                        t2.start()
                    if ret3 is True: 
                        t3 = threading.Thread(target=self.saveImg, args=(path_image3+str(now)+'.jpg', frame3))
                        t3.start()
                    if ret4 is True: 
                        t4 = threading.Thread(target=self.saveImg, args=(path_image4+str(now)+'.jpg', frame4))
                        t4.start()
                    if ret5 is True: 
                        t5 = threading.Thread(target=self.saveImg, args=(path_image5+str(now)+'.jpg', frame5))
                        t5.start()
